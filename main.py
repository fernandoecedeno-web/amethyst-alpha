...
_T_BOOT = _time.perf_counter()

from bot.config import *  # noqa: F403
import bot.config as _bot_config
import bot.execution.trade_lifecycle as _trade_lifecycle_mod
...
from risk import can_open_trade
...
import bot.reporting.console as _console_mod
...
try:
    import kalshi_rich_dashboard as _dashboard_mod
...
_RUNTIME_SENTINEL_VERSION = "RUNTIME_SENTINEL_V1"
HTTP_TIMEOUT_TOTAL = 5
HTTP_RETRY_ATTEMPTS = 2
HTTP_RETRY_BACKOFF = 0.35
...
def rotate_open_position(open_rows, victim_row, replacement, prices, now):
    ...
    _old_entry_meta = _entry_metrics.get((old_ticker, old_side), {})
    _last_exit_meta_by_ticker[old_ticker] = {
        "ts": now,
        "edge": old_edge,
        "spread": safe_float(_old_entry_meta.get("entry_spread"), None),
        "pressure": safe_float(_old_entry_meta.get("entry_pressure"), None),
        "touch": safe_float(_old_entry_meta.get("entry_touch"), None),
        "pnl_pct": pnl_pct,
    }
    log.info(
...
def parse_single_orderbook_payload(ticker, data):
    ...
    if VERBOSE_LOGS:
        log.info(f"[BOOK_DEBUG] ticker={ticker} payload_keys={payload_keys} yes_bid={yes_bid} no_bid={no_bid}")
...
def final_entry_allowed(candidate, regime, global_regime_name: str = ""):
    ...
    if abs(model_prob - 0.50) < 0.015:
        if edge > 0.40 and (spread is None or float(spread) <= 0.02):
            return True, "direction_override", 1.0
    ...
    if abs(yes_pressure - no_pressure) < 0.15:
        if edge >= 0.50 and pressure >= 0.50 and spread is not None and float(spread) <= 0.02 and touch >= 500:
            return True, "breakout_balance_override", 1.0
        if edge >= 0.55 and pressure >= 0.49 and spread is not None and float(spread) <= 0.03 and touch >= 500:
            return True, "strong_edge_side_override", 1.0
        if edge >= 0.40 and pressure >= 0.49 and spread is not None and float(spread) <= 0.03 and touch >= 500:
            return True, "balanced_pressure_override", 1.0
...
def log_skip(ticker, reason, detail=""):
    ...
    if DEBUG:
        from bot.logger import log_skip_compact
        log_skip_compact(log, ticker, reason)


def filter_engine(raw_candidates):
    filtered = []
    r_edge = r_liq = r_spread = 0
    for c in raw_candidates:
        spread = c.get("spread")
        if c.get("edge", 0.0) < MIN_EDGE:
            r_edge += 1
            continue
        if c.get("liquidity_score", 0.0) < MIN_LIQUIDITY_SCORE:
            r_liq += 1
            continue
        if spread is None or spread > FINAL_ENTRY_MAX_SPREAD:
            r_spread += 1
            continue
        filtered.append(c)
    if DEBUG:
        log.info(f"FILTER edge={r_edge} liq={r_liq} spread={r_spread} pass={len(filtered)}")
    return filtered
...
async def get_json_with_retry(session, url, *, headers=None, params=None, source="http", attempts=HTTP_RETRY_ATTEMPTS):
    ...
    for attempt in range(1, attempts + 1):
        try:
            async with session.get(url, headers=headers, params=params, timeout=timeout) as r:
                ...
                body = ((await r.text()) or "")[:160]
                if attempt < attempts and r.status in (429, 500, 502, 503, 504):
                    await asyncio.sleep(HTTP_RETRY_BACKOFF * attempt)
                    continue
                log.warning(
                    f"HTTP_FAIL | src={source} host={host} status={r.status}"
            ...
            if attempt < attempts:
                ...
                continue
            log.warning(
                f"HTTP_FAIL | src={source} host={host}"
                f" exc={type(e).__name__} path={path}"
            )
            return None, None
    return None, None
...
def estimate_model_prob(market, spot_price, minutes_to_expiry, yes_touch=None, no_touch=None):
    ...
    if strike is None or spot_price is None or spot_price <= 0 or strike <= 0:
        return None
    yes_touch = float(yes_touch or 0.0)
    no_touch = float(no_touch or 0.0)
...
async def fetch_market_result(session, ticker):
    ...
    _status, data = await get_json_with_retry(
        session,
        f"{BASE_URL}/markets/{ticker}",
        headers=get_headers(),
        source=f"kalshi_market_detail:{ticker}",
    )
    if _status != 200 or not data:
        return None
    m = data.get("market", {})
    result = m.get("result")
...
async def resolve_trades(session):
    ...
    for trade in trades:
        ...
        _last_exit_meta_by_ticker[trade["ticker"]] = {
            "ts": now,
            "edge": safe_float(trade.get("ev"), 0.0) or 0.0,
            "spread": safe_float(entry_meta.get("entry_spread"), None),
            "pressure": safe_float(entry_meta.get("entry_pressure"), None),
            "touch": safe_float(entry_meta.get("entry_touch"), None),
            "pnl_pct": pnl,
        }
        if rotation_meta:
...
def check_early_exits(prices):
    ...
    for trade in trades:
        ...
        if exit_price is None:
            exit_price = entry_price
        pnl_pct = exit_price - entry_price
        if DEBUG: log.info(f"[EXIT CHECK] {ticker} pnl={pnl_pct:.4f} held={int(age_secs)}")
        reason = None
        ...
        if flat_stop is not None and pnl_pct <= flat_stop:
            ...
            _exit_regime = entry_meta.get("regime", "unknown")
            if (
                pressure_score_now is None or pressure_score_now == 0.0 or
                spread is None or spread == 0.0 or
                _exit_regime in (None, "unknown")
            ):
                log_skip(
                    ticker,
                    "exit_blocked_invalid_state",
                    f"pressure={pressure_score_now}, spread={spread}, regime={_exit_regime}",
                )
            else:
            ...
            if held_secs < 30:
                if DEBUG: log.info(f"[EXIT_GUARD] ticker={ticker} reason=min_hold_window held={int(held_secs)} min=30 block=conviction_decay")
            else:
            ...
            if age_secs < 45:
                if DEBUG: log.info(f"[EXIT_GUARD] ticker={ticker} reason=min_hold_window held={int(age_secs)} min=45 block=time_stop")
            else:
                reason = "time_stop"
                won = 1 if pnl_pct > 0 else 0
        ...
        _last_exit_meta_by_ticker[ticker] = {
            "ts": now,
            "edge": safe_float(entry_meta.get("entry_edge"), safe_float(trade.get("ev"), 0.0) or 0.0) or 0.0,
            "spread": spread if spread is not None else safe_float(entry_meta.get("entry_spread"), None),
            "pressure": pressure_score_now,
            "touch": safe_float(position_intel.get("current_touch"), safe_float(entry_meta.get("entry_touch"), None)),
            "pnl_pct": pnl_pct,
        }
        exit_msg = (
...
async def fetch_spot_prices(session):
    ...
    _status, data = await get_json_with_retry(
        ...
        source="coinbase_btc_spot",
    )
    if _status == 200 and data:
    ...
    for series in PREFERRED_SERIES:
        ...
        while True:
            ...
            _status, data = await get_json_with_retry(
                ...
                headers=get_headers(),
                params=params,
                source=f"kalshi_discovery:{series}",
            )
            if _status == 429:
...
async def fetch_batch_orderbooks(session, tickers):
    ...
    for i, ticker in enumerate(tickers):
        ...
        _status, data = await get_json_with_retry(
            ...
            headers=get_headers(),
            source=f"kalshi_orderbook:{ticker}",
        )
        num_requests += 1
        if _status == 429:
            skip_rate_limited += 1
        ...
        if VERBOSE_LOGS:
            log.info(
                f"[RAW_BOOK] ticker={ticker}"
                f" keys={list(data.keys()) if isinstance(data, dict) else data}"
                f" yes_sample={(data.get('yes') or data.get('yes_dollars')) if isinstance(data, dict) else None}"
                f" no_sample={(data.get('no') or data.get('no_dollars')) if isinstance(data, dict) else None}"
            )
        if VERBOSE_LOGS:
        ...
        if VERBOSE_LOGS:
            log.info(f"[BOOK_BIDS] ticker={ticker} yes_bid={yes_bid} no_bid={no_bid}")
...
async def run_cycle(session, cycle_num):
    ...
    _t_fetch_start = _time.perf_counter() if cycle_num == 1 else None
    ...
    if cycle_num == 1:
        log.info(f"STARTUP_TIMING | first_fetch_all={_time.perf_counter() - _t_fetch_start:.3f}s markets={len(markets)} ob_requests={num_ob_requests}")
    ...
    open_rows, total_open_exposure = rebuild_open_position_state()
    open_count = len(open_rows)
    if DEBUG: log.info(f"[POST-EXIT STATE] active_positions={open_count}")
    if not COMPACT_LOGS:
    ...
    relaxed_adaptive_spread_cap = min(HARD_SPREAD_CEIL + 0.01, adaptive_spread_cap + 0.01)
    ...
    relaxed_min_volume = max(25.0, MIN_VOLUME * 0.35)
    usable_markets = 0
    ...
    def _fmt_metric(value, fmt_spec=".3f", none="-"):
        if value is None:
            return none
        try:
    ...
    def _record_rejection(
        ...
        *,
        ...
        if VERBOSE_LOGS or cycle_num > 1:
            gap_text = f" gap={_fmt_metric(gap, '.4f')}" if gap is not None else ""
            threshold_text = f" threshold={_fmt_metric(threshold, '.4f')}" if threshold is not None else ""
            detail_text = f" detail={detail}" if detail else ""
            log.info(
                yellow(
                    f"[REJECT_DETAIL] ticker={ticker} reason={reason}"
                    f" spread={_fmt_metric(spread)}"
                    f" volume={_fmt_metric(volume, '.0f')}"
                    f" edge={_fmt_metric(edge, '.4f')}"
                    f" mp={_fmt_metric(mp, '.3f')}"
                    f" pressure={_fmt_metric(pressure, '.3f')}"
                    f"{gap_text}{threshold_text}{detail_text}"
                )
            )
        if gap is not None:
            closest_to_passing.append(
                {
                    "ticker": ticker,
                    "reason": reason,
                    "gap": abs(float(gap)),
    ...
    for m in markets:
        ...
        yes_book = {}
        no_book = {}
        current_yes_touch = 0.0
        current_no_touch = 0.0
        ...
        if minutes_to_expiry < 45:
            ...
            _record_rejection(
                ticker,
                "too_soon",
                gap=45 - minutes_to_expiry,
                threshold=45,
                detail=f"minutes={minutes_to_expiry:.0f}",
            )
        ...
        if minutes_to_expiry > SELECTION_MAX_MINUTES:
            ...
            _record_rejection(
                ticker,
                "too_far",
                gap=minutes_to_expiry - SELECTION_MAX_MINUTES,
                threshold=SELECTION_MAX_MINUTES,
                detail=f"minutes={minutes_to_expiry:.0f}",
            )
        ...
        current_yes_touch = float((yes_book or {}).get("touch") or 0.0)
        current_no_touch  = float((no_book  or {}).get("touch") or 0.0)
        ...
        if not yes_book["valid"] and not no_book["valid"]:
            ...
            _record_rejection(
                ticker,
                "no_real_book",
                spread=max([s for s in [quotes.get("yes_spread"), quotes.get("no_spread")] if s is not None], default=None),
                pressure=safe_float(m.get("pressure_score"), 0.0) or 0.0,
                detail=f"yes_touch={current_yes_touch:.0f} no_touch={current_no_touch:.0f}",
            )
        ...
        if not isinstance(yes_book, dict) or not isinstance(no_book, dict):
            log.warning(f"{_RUNTIME_SENTINEL_VERSION}_BOOK_GUARD | ticker={ticker} reason=invalid_book_object")
            skip_model += 1
            _record_rejection(ticker, "invalid_book_state", pressure=pressure_score)
            log_skip(ticker, "invalid_book_state")
            continue
        ...
        if VERBOSE_LOGS:
            log.info(
                f"{_RUNTIME_SENTINEL_VERSION}_PREMODEL | ticker={ticker}"
                f" strike={strike if strike is not None else '-'}"
                f" spot={spot_price if spot_price is not None else '-'}"
                f" yes_touch={current_yes_touch:.0f}"
                f" no_touch={current_no_touch:.0f}"
            )
        if VERBOSE_LOGS:
            log.info(
        ...
        if spot_price is None:
            skip_spot_missing += 1
            _record_rejection(ticker, "no_spot", mp=None, pressure=pressure_score)
            log_skip(ticker, "no_spot")
            continue
        if strike is None:
            skip_model += 1
            _record_rejection(ticker, "no_strike", mp=None, pressure=pressure_score)
            log_skip(ticker, "no_strike")
            continue
        mp = estimate_model_prob(m, spot_price, minutes_to_expiry, current_yes_touch, current_no_touch)
        if mp is None:
            skip_model += 1
            _record_rejection(ticker, "no_strike", mp=None, pressure=pressure_score, detail="model_prob_none")
            log_skip(ticker, "no_strike")
            continue
        ...
        if dist > 0.035:
            skip_model += 1
            _record_rejection(ticker, "far_strike", mp=mp, pressure=pressure_score, gap=dist - 0.035, threshold=0.035, detail=f"dist={dist:.3f}")
        ...
        if volume is not None and volume < relaxed_min_volume and widest_real_spread > MAX_HARD_SPREAD:
            skip_low_volume += 1
            _record_rejection(
                ticker,
                "low_volume",
                spread=widest_real_spread,
                volume=volume,
                mp=mp,
                pressure=pressure_score,
                gap=relaxed_min_volume - volume,
                threshold=relaxed_min_volume,
                detail=f"spread={widest_real_spread:.3f}",
            )
            log_skip(ticker, "low_volume", f"volume={volume:.0f} spread={widest_real_spread:.3f} min={relaxed_min_volume:.0f}")
            continue
        # Edge-based side selection using true execution prices only.
        raw_edge_yes = (mp - yes_ask_f) if yes_book["valid"] and yes_ask_f is not None else None
        raw_edge_no  = ((1.0 - mp) - no_ask_f) if no_book["valid"] and no_ask_f is not None else None
        dist_edge_mult = 1.0
        if dist > 0.04:
            dist_edge_mult = 0.2
        elif dist > 0.03:
            ...
            dist_edge_mult = 0.6
        edge_yes = round(raw_edge_yes * dist_edge_mult, 4) if raw_edge_yes is not None else None
        edge_no  = round(raw_edge_no * dist_edge_mult, 4) if raw_edge_no is not None else None
        if dist_edge_mult < 1.0:
            _raw_edge_best = max([v for v in [raw_edge_yes, raw_edge_no] if v is not None], default=None)
            _adj_edge_best = max([v for v in [edge_yes, edge_no] if v is not None], default=None)
            log.info(
                f"[DISTANCE_ADJUST] ticker={ticker} dist={dist:.4f}"
                f" raw_edge={_raw_edge_best if _raw_edge_best is not None else '-'}"
                f" adj_edge={_adj_edge_best if _adj_edge_best is not None else '-'}"
            )
        ...
        effective_min_edge = round(max(MIN_EDGE - 0.01, effective_min_edge - 0.01), 4)
        yes_touch = current_yes_touch
        no_touch = current_no_touch
        ...
        if eff_edge_yes is not None and eff_edge_yes >= effective_min_edge and (eff_edge_no is None or eff_edge_yes >= eff_edge_no):
            ...
            selected_touch = current_yes_touch
            ...
            selected_touch = current_no_touch
            selection_reason = "stronger_executable_edge"
        else:
            ...
            if (edge_yes is not None) and (edge_no is None or edge_yes >= edge_no):
                ...
                _rspread, _rtouch = yes_spread, current_yes_touch
            elif edge_no is not None:
                _rs, _rp = "no", no_ask_f
                _rraw, _reff, _rrel = edge_no, eff_edge_no, rel_edge_no
                _rspread, _rtouch = no_spread, current_no_touch
            else:
            ...
            if allow_relative_trade:
                ...
                _record_rejection(
                    ticker,
                    "low_edge",
                    spread=max([s for s in [yes_spread, no_spread] if s is not None], default=0.0),
                    volume=volume,
                    edge=_best_edge if _best_edge != -999.0 else None,
                    mp=mp,
                    pressure=pressure_score,
                    gap=(effective_min_edge - _best_edge) if _best_edge != -999.0 else None,
                    threshold=effective_min_edge,
                    detail=f"eff_yes={eff_edge_yes if eff_edge_yes is not None else 'na'} eff_no={eff_edge_no if eff_edge_no is not None else 'na'}",
                )
        ...
        if mp < 0.5:
            if not no_book["valid"]:
                ...
                _record_rejection(
                    ticker,
                    "no_side_not_executable",
                    mp=mp,
                    pressure=pressure_score,
                    edge=eff_edge_no,
                    detail=f"model_no={(1.0 - mp):.3f}",
                )
                log_skip(ticker, "no_side_not_executable", f"mp={mp:.3f} model_no={(1.0 - mp):.3f}")
                continue
            selected_side = "no"
            selected_spread = no_spread
            selected_touch = no_touch
            entry_price = no_ask_f
            raw_edge_sel = edge_no
            true_edge = eff_edge_no
            relative_edge = rel_edge_no
        ...
        if selected_touch < _touch_near_miss_floor:
            ...
            _record_rejection(
                ...
                mp=mp,
                pressure=pressure_score,
                gap=_touch_near_miss_floor - selected_touch,
                threshold=_touch_near_miss_floor,
                detail=f"side={selected_side}",
            )
        ...
        if selected_spread is not None and selected_spread > HARD_SPREAD_CEIL:
            ...
            _record_rejection(
                ...
                mp=mp,
                pressure=pressure_score,
                gap=selected_spread - HARD_SPREAD_CEIL,
                threshold=HARD_SPREAD_CEIL,
                detail=f"side={selected_side}",
            )
            log_skip(ticker, "wide_spread", f"side={selected_side} spread={selected_spread:.3f} ceil={HARD_SPREAD_CEIL:.3f}")
            continue
        if selected_spread is not None and selected_spread > relaxed_adaptive_spread_cap:
            skip_wide_spread += 1
            adapt_near_miss += 1
            if ticker in held_tickers:
                log.info(f"[HELD SKIP] {ticker} reason=spread spread={selected_spread:.3f} cap={relaxed_adaptive_spread_cap:.3f} side={selected_side}")
            if VERBOSE_LOGS: log.info(yellow(f"[NEAR_MISS] ticker={ticker} reason=spread value={selected_spread:.4f} threshold={relaxed_adaptive_spread_cap:.4f}"))
            _record_rejection(
                ticker,
                "wide_spread",
                spread=selected_spread,
                ...
                mp=mp,
                pressure=pressure_score,
                gap=selected_spread - relaxed_adaptive_spread_cap,
                threshold=relaxed_adaptive_spread_cap,
                detail=f"side={selected_side}",
            )
            log_skip(ticker, "wide_spread", f"side={selected_side} spread={selected_spread:.3f} cap={relaxed_adaptive_spread_cap:.3f}")
            continue
        adapt_spread_pass += 1
        ...
        if not _qf_ok:
            ...
            _record_rejection(
                ticker,
                f"pre_filter_{_qf_reason}",
                spread=selected_spread,
                volume=volume,
                edge=true_edge,
                mp=mp,
                pressure=pressure_score,
                detail=f"side={selected_side} yes_touch={_yt:.0f} no_touch={_nt:.0f} prelim_q={_prelim_quality:.3f}",
            )
        ...
        if selected_side == "yes" and mp >= 0.68 and pressure_score >= 0.55:
            ...
            _record_rejection(ticker, "late_chase", spread=selected_spread, volume=volume, edge=true_edge, mp=mp, pressure=pressure_score, detail="high_mp_high_pressure_yes")
        ...
        if selected_spread > 0.03 and pressure_score >= 0.55:
            ...
            _record_rejection(ticker, "late_chase", spread=selected_spread, volume=volume, edge=true_edge, mp=mp, pressure=pressure_score, detail="wide_spread_high_pressure")
        ...
        _exec_quality_reason = None
        if selected_spread >= 0.04:
            if true_edge < 0.50 or pressure_score < 0.50 or selected_touch < max(50.0, adaptive_touch_req * 2.0):
                _exec_quality_reason = "elevated_spread_exec"
        elif selected_spread >= 0.03:
            if true_edge < 0.42 or pressure_score < 0.49 or selected_touch < max(25.0, adaptive_touch_req * 1.5):
                _exec_quality_reason = "weak_execution_quality"
        if _exec_quality_reason is not None:
            skip_entry_score += 1
            if DEBUG:
                log.info(yellow(
                    f"[ENTRY_QUAL_BLOCK] ticker={ticker} reason={_exec_quality_reason}"
                    f" edge={true_edge:.3f} spread={selected_spread:.3f}"
                    f" liq={liq_score:.3f} pressure={pressure_score:.3f}"
                    f" sidectl={abs((safe_float(m.get('pressure_score'), 0.0) or 0.0) - (safe_float(m.get('no_pressure_score'), 0.0) or 0.0)):.3f}"
            ...
            _record_rejection(
                ...
                mp=mp,
                pressure=pressure_score,
                detail=f"reason={_exec_quality_reason} touch={selected_touch:.0f}",
            )
            log_skip(ticker, "weak_execution_quality", f"reason={_exec_quality_reason} spread={selected_spread:.3f} touch={selected_touch:.0f}")
            continue
        ...
        elif side_control_delta < 0.15 and true_edge < 0.50:
        ...
        if entry_score < EXEC_SCORE_THRESHOLD and pressure_score < 0.45:
            ...
            _record_rejection(
                ticker,
                "mid_signal_low_pressure",
                spread=selected_spread,
                volume=volume,
                edge=true_edge,
                mp=mp,
                pressure=pressure_score,
                gap=0.45 - pressure_score,
                threshold=0.45,
                detail=f"entry_score={entry_score:.3f}",
            )
        ...
        if bad_regime and quality_score < 0.35:
            ...
            _record_rejection(
                ticker,
                "bad_regime_low_quality",
                spread=selected_spread,
                volume=volume,
                edge=true_edge,
                mp=mp,
                pressure=pressure_score,
                gap=0.35 - quality_score,
                threshold=0.35,
                detail=f"side={selected_side}",
            )
        ...
        if entry_price < 0.06:
            ...
            _record_rejection(ticker, "no_tradeable_side", spread=selected_spread, volume=volume, edge=true_edge, mp=mp, pressure=pressure_score, gap=0.06 - entry_price, threshold=0.06, detail=f"side={selected_side}")
            log_skip(ticker, "no_tradeable_side", f"entry_price={entry_price:.3f} side={selected_side} floor=0.06")
            continue
        if entry_price < 0.10 and not (true_edge >= 0.60 and dist <= 0.02):
            skip_price += 1
            _record_rejection(
                ticker,
                "no_tradeable_side",
                ...
                mp=mp,
                pressure=pressure_score,
                gap=0.10 - entry_price,
                threshold=0.10,
                detail=f"side={selected_side} dist={dist:.3f}",
            )
            log_skip(ticker, "no_tradeable_side", f"entry_price={entry_price:.3f} side={selected_side} floor=0.10 dist={dist:.3f}")
            continue
        ...
        if entry_price < price_floor:
            if true_edge >= 0.30 and quality_score >= 0.65 and elite_override_allowed:
                ...
                _vol_s = f"{int(volume)}" if volume is not None else "-"
                if VERBOSE_LOGS: log.info(f"[GATE] {ticker} sp={selected_spread:.3f} vol={_vol_s} yask={yes_ask_f if yes_ask_f is not None else '-'} nask={no_ask_f if no_ask_f is not None else '-'} mpy={mp:.3f} mpn={1-mp:.3f} ey={eff_edge_yes if eff_edge_yes is not None else '-'} en={eff_edge_no if eff_edge_no is not None else '-'} pr={pressure_score:.3f} vel=- elite=- escore=- side={selected_side} block=price({entry_price:.3f})")
                _record_rejection(ticker, "no_tradeable_side", spread=selected_spread, volume=volume, edge=true_edge, mp=mp, pressure=pressure_score, gap=price_floor - entry_price, threshold=price_floor, detail=f"side={selected_side}")
        ...
        if selected_side == "yes" and entry_price > 0.20:
            ...
            _vol_s = f"{int(volume)}" if volume is not None else "-"
            if VERBOSE_LOGS: log.info(f"[GATE] {ticker} sp={selected_spread:.3f} vol={_vol_s} yask={yes_ask_f if yes_ask_f is not None else '-'} nask={no_ask_f if no_ask_f is not None else '-'} mpy={mp:.3f} mpn={1-mp:.3f} ey={eff_edge_yes if eff_edge_yes is not None else '-'} en={eff_edge_no if eff_edge_no is not None else '-'} pr={pressure_score:.3f} vel=- elite=- escore=- side={selected_side} block=price({entry_price:.3f})")
            _record_rejection(ticker, "no_tradeable_side", spread=selected_spread, volume=volume, edge=true_edge, mp=mp, pressure=pressure_score, gap=entry_price - 0.20, threshold=0.20, detail="side=yes")
        ...
        if entry_price > price_cap:
            ...
            _vol_s = f"{int(volume)}" if volume is not None else "-"
            if VERBOSE_LOGS: log.info(f"[GATE] {ticker} sp={selected_spread:.3f} vol={_vol_s} yask={yes_ask_f if yes_ask_f is not None else '-'} nask={no_ask_f if no_ask_f is not None else '-'} mpy={mp:.3f} mpn={1-mp:.3f} ey={eff_edge_yes if eff_edge_yes is not None else '-'} en={eff_edge_no if eff_edge_no is not None else '-'} pr={pressure_score:.3f} vel=- elite=- escore=- side={selected_side} block=price({entry_price:.3f})")
            _record_rejection(ticker, "no_tradeable_side", spread=selected_spread, volume=volume, edge=true_edge, mp=mp, pressure=pressure_score, gap=entry_price - price_cap, threshold=price_cap, detail=f"side={selected_side}")
        ...
        if not ki_pass:
            ...
            _record_rejection(
                ...
                mp=candidate.get("mp"),
                pressure=candidate.get("pressure_score"),
                gap=max(0.0, _ki_th - _ki_ks),
                threshold=_ki_th,
            )
    ...
    _raw_count = len(candidates)
    filtered = filter_engine(candidates)
    family_runtime_stats = {}
    for row in open_rows:
        try:
            _fam = row.get("family", "OTHER")
            _side = row.get("side", "")
            _entry_meta = _entry_metrics.get((row.get("ticker", ""), _side), {})
            _intel = compute_position_intel(row, _side, prices, _entry_meta)
            _stat = family_runtime_stats.setdefault(
                _fam,
                {"count": 0, "pnl_sum": 0.0, "losers": 0, "best_edge": 0.0, "best_pressure": 0.0, "tightest_spread": 1.0},
            )
            _stat["count"] += 1
            _pnl_pct = safe_float(_intel.get("current_pnl_pct"), 0.0) or 0.0
            _stat["pnl_sum"] += _pnl_pct
            if _pnl_pct < 0.0:
                _stat["losers"] += 1
            _stat["best_edge"] = max(_stat["best_edge"], safe_float(_intel.get("current_edge"), 0.0) or 0.0)
            _stat["best_pressure"] = max(_stat["best_pressure"], safe_float(_intel.get("current_pressure_score"), 0.0) or 0.0)
            _stat["tightest_spread"] = min(_stat["tightest_spread"], max(0.0, safe_float(_intel.get("current_spread"), 1.0) or 1.0))
        except Exception:
            continue
    for _stat in family_runtime_stats.values():
        _stat["avg_pnl_pct"] = _stat["pnl_sum"] / max(1, _stat["count"])
    for c in filtered:
        base_priority = execution_priority_score(c)
        family_penalty = 0.0
        _fam_state = family_runtime_stats.get(c.get("family", "OTHER"))
        if _fam_state and _fam_state.get("avg_pnl_pct", 0.0) <= -0.02:
            family_penalty += 0.04
            if _fam_state.get("losers", 0) > 0 and (c.get("spread") or 0.0) > 0.02:
                family_penalty += 0.02
        if c.get("ticker") in recent_losses:
            family_penalty += 0.03
        c["family_penalty"] = round(family_penalty, 4)
        c["execution_priority"] = round(base_priority * 0.85 + c.get("killer_score", 0.5) * 0.15 - family_penalty, 4)
    ...
    if DEBUG:
        log.info(
            f"PIPE raw={usable_markets} killer={killer_passed} filter={len(filtered)}"
            f" ranked={len(ranked_candidates)} top={min(5, len(ranked_candidates))}"
        )
    if VERBOSE_LOGS:
    ...
    from bot.logger import log_cycle, log_top
    _risk_pct = total_open_exposure / BANKROLL if BANKROLL else 0.0
    log_cycle(log, cycle_num, BANKROLL + _realized_cash_pnl, len(open_rows), _risk_pct * 100)
    if not COMPACT_LOGS:
    ...
    if COMPACT_LOGS:
        ...
        if DEBUG:
            log.info(f"TOP DEBUG count={len(_top_candidates)} sample={_top_candidates[:2]}")
        log_top(log, _top_candidates, limit=5)
    near_pass_override_tickers = []
    _near_pass_seen = set()
    for miss in sorted(closest_to_passing, key=lambda x: x["gap"]):
        _t = miss.get("ticker")
        if not _t or _t in _near_pass_seen:
            continue
        _near_pass_seen.add(_t)
        near_pass_override_tickers.append(_t)
        if len(near_pass_override_tickers) >= 2:
            break
    near_pass_override_used = 0

    def _record_candidate_skip(c, reason, *, detail="", gap=None, threshold=None):
        _record_rejection(
            c["ticker"],
            reason,
            spread=c.get("spread"),
    ...
    for c in ranked_candidates:
        ...
        if c.get("allocation_score", float("-inf")) < effective_min_alloc_score:
            _record_candidate_skip(
                c,
                "alloc_score_too_low",
                detail=f"alloc={c.get('allocation_score', float('-inf')):.3f}",
                gap=effective_min_alloc_score - c.get("allocation_score", float("-inf")),
                threshold=effective_min_alloc_score,
            )
            continue
        ts_key = (c["ticker"], c["side"])
        if c["ticker"] in open_tickers:
            if not regime_action["allow_reinforce"]:
                if DEBUG: log.info(yellow(f"[REINFORCE_BLOCKED] ticker={c['ticker']} reason=global_regime_{global_regime['regime'].lower()}"))
                if _dashboard is not None:
                    _dashboard.add_event("REINFORCE_BLOCKED", c["ticker"], f"regime={global_regime['regime']}")
                _record_candidate_skip(c, f"reinforce_blocked_{global_regime['regime'].lower()}", detail="reinforce_not_allowed")
            ...
            if not reinforce_ok:
                ...
                if reinforce_block_reason:
                    ...
                    _record_candidate_skip(c, f"reinforce_{reinforce_block_reason}")
            if reinforce_ok:
                ...
                if reinforce_size >= 0.5:
                    ...
                    buys += 1
                    continue
                if DEBUG: log.info(yellow(f"[REINFORCE BLOCKED] ticker={c['ticker']} reason=insufficient_cap size={reinforce_size:.2f}"))
                _record_candidate_skip(c, "size_too_small", detail=f"reinforce_size={reinforce_size:.2f}")
            else:
                log.info(f"[ENTRY BLOCKED] already holding {c['ticker']} open={len(open_rows)}/{MAX_OPEN_POSITIONS}")
                if len(open_rows) >= MAX_OPEN_POSITIONS:
                    _record_candidate_skip(c, "already_holding")
            skipped_tickers.add(c["ticker"])
            continue
        ...
        if not regime_action["allow_new_entries"]:
            ...
            if not _mid_tier_global:
                if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=global_regime_{global_regime['regime'].lower()}"))
                _record_candidate_skip(c, f"global_regime_{global_regime['regime'].lower()}")
                skipped_tickers.add(c["ticker"])
                continue
        ...
        if len(open_rows) >= MAX_OPEN_POSITIONS:
            if not regime_action["allow_rotation"] or not ROTATION_ENABLED:
                ...
                if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason={reason}"))
                _record_candidate_skip(
                    c,
                    "portfolio_full" if reason == "rotation_disabled" else reason,
                    detail=f"open_cap current_open={len(open_rows)}",
                )
                skipped_tickers.add(c["ticker"])
                continue
            rotation_checked += 1
            ...
            if victim is None:
                blocked_open_cap += 1
                if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=open_cap current_open={len(open_rows)}"))
                if force_rotation_eval:
                    log.info(yellow(f"[ROTATION_MISSED] ticker={c['ticker']} edge={c['edge']:.3f} reason=no_victim"))
                _record_candidate_skip(c, "portfolio_full", detail=f"open_cap current_open={len(open_rows)}")
                skipped_tickers.add(c["ticker"])
                continue
            ...
            if upgrade_reason == "rotation_suppressed_long_horizon":
                ...
                _record_candidate_skip(c, "rotation_suppressed_long_horizon")
                skipped_tickers.add(c["ticker"])
                continue
            log.info(magenta(
            ...
            if c["edge"] < victim_edge and c["quality_score"] < victim_quality and not force_rotation:
                ...
                if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=no_downgrade"))
                if force_rotation_eval:
                    log.info(yellow(f"[ROTATION_MISSED] ticker={c['ticker']} edge={c['edge']:.3f} reason=no_downgrade"))
                _record_candidate_skip(c, "no_downgrade")
                skipped_tickers.add(c["ticker"])
                continue
            if victim_edge >= 0.20 and victim_quality >= 0.55 and victim_held_secs < 600 and not force_rotation:
                ...
                _record_candidate_skip(c, "conviction_hold")
                skipped_tickers.add(c["ticker"])
                continue
            if position_expiry_progress(victim_row, now) >= 0.80 and victim_pnl_pct > 0 and victim_pressure >= 0.40 and not force_rotation:
                ...
                if DEBUG: log.info(yellow(f"[BLOCKED] reason=late_cycle_lock ticker={victim_row.get('ticker','')}"))
                if force_rotation_eval:
                    log.info(yellow(f"[ROTATION_MISSED] ticker={c['ticker']} edge={c['edge']:.3f} reason=late_cycle_lock"))
                _record_candidate_skip(c, "late_cycle_lock")
                skipped_tickers.add(c["ticker"])
                continue
            if c["spread"] > adaptive_spread_cap or c["selected_touch"] < adaptive_touch_req:
                if not force_rotation:
                    ...
                    _record_candidate_skip(c, "rotation_confidence")
                    skipped_tickers.add(c["ticker"])
                    continue
            if victim_held_secs < 180 and victim_pnl_pct > -0.01 and rotation_score < 0.15 and not force_rotation:
                ...
                _record_candidate_skip(c, "rotation_churn_guard")
                skipped_tickers.add(c["ticker"])
                continue
            if rotations >= MAX_ROTATIONS_PER_CYCLE and not forced_upgrade and not force_rotation:
                ...
                _record_candidate_skip(c, "rotation_budget")
                skipped_tickers.add(c["ticker"])
                continue
            if rotation_score < rotation_threshold and not force_rotation:
                ...
                _record_candidate_skip(c, "portfolio_full", detail=f"rotation_score={rotation_score:.4f} threshold={rotation_threshold:.4f}")
                skipped_tickers.add(c["ticker"])
                continue
        ...
        if rotation_candidate is None and new_trades_this_cycle >= max_new_trades_this_cycle:
            skip_cycle_cap += 1
            _record_candidate_skip(c, "portfolio_full", detail=f"max_new={max_new_trades_this_cycle}")
        ...
        if rotation_candidate is None and c.get("tier_name") == "T3" and t3_this_cycle >= MAX_T3_PER_CYCLE:
            skip_cycle_cap += 1
            _record_candidate_skip(c, "portfolio_full", detail=f"max_t3={MAX_T3_PER_CYCLE}")
            log_skip(c["ticker"], "t3_cap", f"max_t3={MAX_T3_PER_CYCLE}")
            skipped_tickers.add(c["ticker"])
            continue
        if c["quality_score"] < 0.40 and c["edge"] < 0.18:
            skip_entry_score += 1
            if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=weak_combo edge={c['edge']:.3f} quality={c['quality_score']:.3f}"))
            _record_candidate_skip(c, "weak_combo")
            skipped_tickers.add(c["ticker"])
            continue
        key = c["family"]
        current_cluster_count = cluster_counts.get(key, 0)
        family_state = family_runtime_stats.get(key)
        ...
        if (
            family_state
            and current_cluster_count >= 1
            and family_state.get("avg_pnl_pct", 0.0) <= -0.02
            and not hard_elite
        ):
            family_upgrade = (
                c["edge"] >= family_state.get("best_edge", 0.0) + 0.08
                or c["pressure_score"] >= family_state.get("best_pressure", 0.0) + 0.04
                or c["spread"] <= max(0.02, family_state.get("tightest_spread", 1.0) - 0.01)
            )
            if not family_upgrade:
                skip_cluster += 1
                if DEBUG:
                    log.info(yellow(
                        f"[CONCENTRATION_BLOCK] ticker={c['ticker']} family={key}"
                        f" avg_pnl={family_state.get('avg_pnl_pct', 0.0):.3f}"
                        f" edge={c['edge']:.3f} spread={c['spread']:.3f}"
                        f" pressure={c['pressure_score']:.3f}"
                    ))
                _record_candidate_skip(c, "concentration_block", detail=f"family={key} avg_pnl={family_state.get('avg_pnl_pct', 0.0):.3f}")
                skipped_tickers.add(c["ticker"])
                continue
        ...
        if current_cluster_count >= MAX_CLUSTER_HARD_CAP and not diversify_force and not hard_elite:
            skip_cluster += 1
            if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=cluster_hard_cap current={current_cluster_count} cap={MAX_CLUSTER_HARD_CAP}"))
            _record_candidate_skip(c, "risk_cap_exceeded", detail=f"cluster_hard_cap current={current_cluster_count} cap={MAX_CLUSTER_HARD_CAP}")
        ...
        if current_cluster_count >= MAX_POSITIONS_PER_CLUSTER:
            if not high_edge_override and not diversify_force and not hard_elite:
                skip_cluster += 1
                if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=cluster_exposure current={current_cluster_count} cap={MAX_POSITIONS_PER_CLUSTER}"))
                _record_candidate_skip(c, "risk_cap_exceeded", detail=f"cluster_exposure current={current_cluster_count} cap={MAX_POSITIONS_PER_CLUSTER}")
        ...
        if not OFFLINE_LIFECYCLE_TEST and c["ticker"] in _entry_cooldown_by_ticker and now < _entry_cooldown_by_ticker[c["ticker"]]:
            skip_cooldown += 1
            _record_candidate_skip(c, "cooldown")
        ...
        last_exit_spread = safe_float(last_exit_meta.get("spread"), None) if last_exit_meta else None
        last_exit_pressure = safe_float(last_exit_meta.get("pressure"), None) if last_exit_meta else None
        last_exit_touch = safe_float(last_exit_meta.get("touch"), None) if last_exit_meta else None
        prev_edge = (safe_float(last_exit_meta.get("edge"), 0.0) or 0.0) if last_exit_meta else 0.0
        edge_improved = (not last_exit_meta) or c["edge"] >= prev_edge + 0.08
        structure_better = (
            not last_exit_meta
            or (last_exit_spread is not None and c["spread"] <= max(0.02, last_exit_spread - 0.01))
            or (last_exit_pressure is not None and c["pressure_score"] >= last_exit_pressure + 0.04)
            or (last_exit_touch is not None and c["selected_touch"] >= max(last_exit_touch * 1.5, last_exit_touch + 100.0))
        )
        reentry_ok = c["spread"] <= 0.03 and price_changed and edge_improved and structure_better
        if not OFFLINE_LIFECYCLE_TEST and last_exit_meta and (now - last_exit_meta["ts"]).total_seconds() < 90:
            if c["edge"] < prev_edge + 0.05:
                _strong_reentry = (
                    float(c.get("edge", 0.0)) >= max(0.50, prev_edge + 0.08)
                    and c.get("spread") is not None and float(c.get("spread") or 1.0) <= 0.02
                    and float(c.get("pressure_score", 0.0)) >= 0.49
                    and float(c.get("selected_touch", 0.0) or 0.0) >= max(500.0, float(last_exit_touch or 0.0))
                )
                if _strong_reentry:
                    log.info(cyan(
                        f"[OVERRIDE_ENTRY] ticker={c['ticker']} reason=reentry_cooldown_override"
                        f" edge={c['edge']:.3f} spread={c['spread']:.3f}"
                        f" pressure={c['pressure_score']:.3f} prev_edge={prev_edge:.3f}"
                    ))
                else:
                    ...
                    if DEBUG:
                        log.info(yellow(
                            f"[REENTRY_BLOCK] ticker={c['ticker']} cooldown=1"
                            f" edge={c['edge']:.3f} spread={c['spread']:.3f}"
                            f" pressure={c['pressure_score']:.3f} improved={1 if edge_improved and structure_better else 0}"
                        ))
                    _record_candidate_skip(c, "fresh_reentry_cooldown", detail=f"prev_edge={prev_edge:.3f}")
                    skipped_tickers.add(c["ticker"])
                    continue
        if not OFFLINE_LIFECYCLE_TEST and recent_loss_ts and time_since_exit is not None and time_since_exit < 15:
            skip_reentry_blocked += 1
            _record_candidate_skip(c, "cooldown_short")
        ...
        if not OFFLINE_LIFECYCLE_TEST and recent_loss_ts and not reentry_ok:
            ...
            if DEBUG:
                log.info(yellow(
                    f"[REENTRY_BLOCK] ticker={c['ticker']} cooldown=0"
                    f" edge={c['edge']:.3f} spread={c['spread']:.3f}"
                    f" pressure={c['pressure_score']:.3f}"
                    f" price_changed={1 if price_changed else 0} edge_improved={1 if edge_improved else 0}"
                    f" structure_better={1 if structure_better else 0}"
                ))
            _record_candidate_skip(c, "reentry_blk", detail=f"price_changed={1 if price_changed else 0} edge_improved={1 if edge_improved else 0} structure_better={1 if structure_better else 0}")
        ...
        if ts_key in open_trades_index:
            ...
            if not (elite_ok or pr_ok or mom_ok):
                skip_reentry_blocked += 1
                _record_candidate_skip(c, "reentry_not_improved")
                skipped_tickers.add(c["ticker"])
                continue
        cash_balance = current_cash_balance(total_open_exposure)
        trade_cap = cash_balance * MAX_CAPITAL_PER_TRADE_PCT
        cycle_cap_remaining = max(0.0, cycle_cap_limit - cycle_cap_used)
        if not COMPACT_LOGS: log_capital_state("CAPITAL", total_open_exposure, cycle_cap_limit, cycle_cap_used, trade_cap)
        ...
        if cur_exposure >= EXPOSURE_CAP:
            skip_exposure += 1
            if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=exposure_cap current_exposure={total_open_exposure:.2f} cap={EXPOSURE_CAP:.2f}"))
            _record_candidate_skip(c, "risk_cap_exceeded", detail=f"exposure_cap current={total_open_exposure:.2f} cap={EXPOSURE_CAP:.2f}")
            skipped_tickers.add(c["ticker"])
            continue
        ...
        if fam_exp.get(c["family"], 0.0) + c["size"] > CAP_FAMILY_PCT * BANKROLL:
            ...
            if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=family_exposure family={c['family']} family_exposure={fam_exp.get(c['family'], 0.0):.2f}"))
            _record_candidate_skip(c, "risk_cap_exceeded", detail=f"family_exposure family={c['family']} current={fam_exp.get(c['family'], 0.0):.2f}")
            skipped_tickers.add(c["ticker"])
            continue
        if c["hours"] <= 1 and short_exp + c["size"] > CAP_SHORT_PCT * BANKROLL:
            skip_bucket_exposure += 1
            if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=short_bucket_exposure short_exposure={short_exp:.2f}"))
            _record_candidate_skip(c, "risk_cap_exceeded", detail=f"short_bucket_exposure current={short_exp:.2f}")
            skipped_tickers.add(c["ticker"])
            continue
        if c["hours"] > 1 and long_exp + c["size"] > CAP_LONG_PCT * BANKROLL:
            skip_bucket_exposure += 1
            if DEBUG: log.info(yellow(f"[BLOCKED] ticker={c['ticker']} reason=long_bucket_exposure long_exposure={long_exp:.2f}"))
            _record_candidate_skip(c, "risk_cap_exceeded", detail=f"long_bucket_exposure current={long_exp:.2f}")
            skipped_tickers.add(c["ticker"])
            continue
        if c["edge"] < MIN_EDGE:
            skip_edge += 1
            _record_candidate_skip(c, "edge_gate", gap=MIN_EDGE - c["edge"], threshold=MIN_EDGE)
            log_skip(c["ticker"], "edge_gate", f"edge={c['edge']:.3f}")
            skipped_tickers.add(c["ticker"])
            continue
        _risk_state = {"bankroll": BANKROLL + _realized_cash_pnl, "open_positions": len(open_rows)}
        _risk_ok, _risk_reason = can_open_trade(c, _risk_state)
        if not _risk_ok:
            _record_candidate_skip(c, _risk_reason)
            log_skip(c["ticker"], _risk_reason)
            skipped_tickers.add(c["ticker"])
            continue
        attempts += 1
        if rotation_candidate is not None:
        ...
        if final_entry_ok and final_entry_reason == "direction_override":
            log.info(cyan(
                f"[OVERRIDE_ENTRY] ticker={c['ticker']} reason=direction_override"
                f" mp={c.get('mp', 0.0):.3f} edge={c['edge']:.3f} spread={c['spread']:.3f}"
            ))
        if final_entry_ok and final_entry_reason == "balanced_pressure_override":
            log.info(cyan(
                f"[OVERRIDE_ENTRY] ticker={c['ticker']} reason=balanced_pressure_override"
                f" pressure={c['pressure_score']:.3f} edge={c['edge']:.3f} spread={c['spread']:.3f}"
            ))
        if final_entry_ok and final_entry_reason == "breakout_balance_override":
            log.info(cyan(
                f"[OVERRIDE_ENTRY] ticker={c['ticker']} reason=breakout_balance_override"
                f" pressure={c['pressure_score']:.3f} edge={c['edge']:.3f} spread={c['spread']:.3f}"
            ))
        if not final_entry_ok:
            _is_low_edge_block = final_entry_reason in ("low_edge", "final_low_edge")
            near_pass_override = (
                near_pass_override_used < 2
                and final_entry_reason in ("weak_pressure", "low_pressure_gate", "low_edge", "final_low_edge")
                # low_edge tickers pass the scan loop so they never appear in near_pass_override_tickers;
                # bypass that membership check and rely on direct edge/spread guards instead.
                and (c["ticker"] in near_pass_override_tickers or _is_low_edge_block)
                and float(c.get("edge", 0.0)) > 0.35
                and c.get("spread") is not None
                and float(c.get("spread") or 0.0) <= 0.02
            )
            if near_pass_override:
                near_pass_override_used += 1
                final_entry_ok = True
                final_entry_reason = "near_pass_activation"
                log.info(cyan(
                    f"[OVERRIDE_ENTRY] ticker={c['ticker']} reason=near_pass_activation"
                    f" orig_block={('low_edge' if _is_low_edge_block else 'pressure_gate')}"
                    f" edge={c['edge']:.3f} spread={c['spread']:.3f}"
                    f" pressure={c['pressure_score']:.3f}"
                ))
            if final_entry_ok:
                ...
                last_final_rejection_reason = final_entry_reason
                _record_rejection(
                    c["ticker"],
                    f"final_{final_entry_reason}",
                    spread=c.get("spread"),
                    volume=None,
                    edge=c.get("edge"),
                    mp=c.get("mp"),
                    pressure=c.get("pressure_score"),
                    detail=f"regime={c.get('regime', '-')}",
                )
                log_skip(
                    c["ticker"],
                    "final_entry_gate",
        ...
        if DEBUG:
            log.info(cyan(
                f"[ENTRY_APPROVED] ticker={c['ticker']}"
                f" edge={c['edge']:.3f} spread={c['spread']:.3f}"
                f" liq={c['liquidity_score']:.3f} pressure={c['pressure_score']:.3f}"
                f" sidectl={abs(c.get('yes_pressure_score', c['pressure_score']) - c.get('no_pressure_score', 0.0)):.3f}"
                f" cluster={current_cluster_count}"
            ))
        from bot.logger import log_enter
        log_enter(log, c['ticker'], c['side'], c['edge'])
        signals += 1
        if c["tier"] == 1:
            tier1_signals += 1
        else:
    ...
    _reject_parts = " ".join(f"{k}={v}" for k, v in sorted(rejection_counts.items(), key=lambda kv: (-kv[1], kv[0])))
    log.info(cyan(f"REJECTION_SUMMARY | {_reject_parts if _reject_parts else 'none=0'}"))
    if closest_to_passing:
        for idx, miss in enumerate(sorted(closest_to_passing, key=lambda x: x["gap"])[:3], 1):
            _detail = miss.get("detail") or ""
            log.info(
                cyan(
                    f"NEAR_PASS_{idx} | ticker={miss['ticker']} reason={miss['reason']}"
                    f" gap={miss['gap']:.4f} spread={_fmt_metric(miss.get('spread'))}"
                    f" volume={_fmt_metric(miss.get('volume'), '.0f')}"
                    f" edge={_fmt_metric(miss.get('edge'), '.4f')}"
                    f" mp={_fmt_metric(miss.get('mp'), '.3f')}"
                    f" pressure={_fmt_metric(miss.get('pressure'), '.3f')}"
                    f"{f' detail={_detail}' if _detail else ''}"
                )
            )
    if cycle_num % 10 == 0:
    ...
    if _dashboard is not None:
        ...
        _dashboard.set_idle(CYCLE_SECS)
    if cycle_num == 1:
        log.info(f"STARTUP_TIMING | cycle1_total={_time.perf_counter() - _t_fetch_start:.3f}s total_from_boot={_time.perf_counter() - _T_BOOT:.3f}s")
    persist_runtime_state()

async def main():
    global _offline_lifecycle_cycle, _dashboard
    _t_main_start = _time.perf_counter()
    _runtime_ts = utc_now().isoformat()
    _runtime_pid = os.getpid()
    _mode_label = "offline" if OFFLINE_MODE else "live"
    log.info(
        f"{_RUNTIME_SENTINEL_VERSION} | ts={_runtime_ts} pid={_runtime_pid}"
        f" file={Path(__file__).resolve()}"
    )
    log.info(f"STARTUP_TIMING | import_to_main={_t_main_start - _T_BOOT:.3f}s")
    if VERBOSE_LOGS:
        log.info(
            f"RUNTIME_ENV | cwd={os.getcwd()} exe={sys.executable}"
            f" sys_path0_5={sys.path[:5]}"
            f" bot.config={Path(_bot_config.__file__).resolve()}"
            f" analytics={Path(_analytics.__file__).resolve()}"
            f" lifecycle={Path(_trade_lifecycle_mod.__file__).resolve()}"
            f" console={Path(_console_mod.__file__).resolve()}"
            f" dashboard={(Path(_dashboard_mod.__file__).resolve() if _dashboard_mod is not None else 'unavailable')}"
        )
    log.info(
        f"RUNTIME_CONFIG | base_url={BASE_URL} mode={_mode_label}"
        f" pid={_runtime_pid}"
    )
    if VERBOSE_LOGS:
        log.info(
            f"RUNTIME_HTTP | shared_session=1 timeout_total={HTTP_TIMEOUT_TOTAL}"
            f" retry_attempts={HTTP_RETRY_ATTEMPTS}"
        )
    if _DASHBOARD_AVAILABLE:
        ...
        _dashboard.add_event("SYSTEM", "BOOT", f"bankroll=${BANKROLL:.2f} mode={'OFFLINE' if OFFLINE_MODE else 'LIVE'}")
    print_cycle_header(
        log,
    ...
    if OFFLINE_DEBUG:
        ...
        return
    log.info(f"STARTUP_TIMING | init_complete={_time.perf_counter() - _t_main_start:.3f}s total_to_scan={_time.perf_counter() - _T_BOOT:.3f}s")
    cycle=0
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=HTTP_TIMEOUT_TOTAL)) as session:
            while True:
                cycle+=1
                try:
